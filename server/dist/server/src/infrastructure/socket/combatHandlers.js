...
function registerCombatHandlers(io, socket, gameRoomManager) {
    socket.on('combat_choice', ({ choice, siegeCost }) => {
        ...
        if (gameRoomManager.isCombatReady(code)) {
            ...
            let finalAction = 'FIGHT';
            if (attackerChoice === 'RETREAT')
                finalAction = 'RETREAT';
            else if (defenderChoice === 'RETREAT_CITY')
                finalAction = 'RETREAT_CITY';
            else if (attackerChoice === 'SIEGE')
                finalAction = 'SIEGE';
            else
                finalAction = 'FIGHT';
            ...
            console.log(`[COMBAT_HANDLER] stateToResolve.humanFactions = ${JSON.stringify(stateToResolve.humanFactions)}`);
            const updates = (0, combat_1.resolveCombatResult)(stateToResolve, finalAction, finalSiegeCost);
            ...
            if (nextCombat) {
                ...
                if (!attackerIsHuman && !defenderIsHuman) {
                    ...
                    const autoUpdates = (0, combat_1.resolveCombatResult)(room.gameState, 'FIGHT', 0);
            ...
            const clientState = (0, gameLogic_1.getClientState)(rawStateForBroadcast);
            ...
            if (phaseInfo.active) {
                ...
                if (nextCombat || (room.gameState.combatQueue && room.gameState.combatQueue.length > 0)) {
                    ...
                    (0, battlePhaseUtils_1.emitCombatPhaseUpdate)(io, code, nextCombat, room.gameState.combatQueue || [], room.gameState, gameRoomManager.getBattlePhaseInfo(code).resolved);
                }
                else {
