...
exports.executeRepublicanEarlyGame = executeRepublicanEarlyGame;
 ...
 */
function executeRepublicanEarlyGame(state, faction, budget) {
    // SECURITY CHECK: Verify we are not running this for a human player
    const isHuman = state.playerFactions?.includes(faction) || state.playerFaction === faction;
    if (isHuman) {
    ...
    const enemiesInHome = updates.armies.some(a => a.faction !== faction &&
 ...
 */
function retreatGarrisons(state, faction, updates) {
    const positions = [DEFENSIVE_POSITIONS.HEATHERFIELD, DEFENSIVE_POSITIONS.LIGHTHOUSE];
    for (const pos of positions) {
        const garrison = updates.armies.find(a => a.faction === faction &&
            a.roadId === pos.roadId &&
 ...
 */
function cleanupOrphanConstructions(state, faction, updates) {
    const positions = [DEFENSIVE_POSITIONS.HEATHERFIELD, DEFENSIVE_POSITIONS.LIGHTHOUSE];
    for (const pos of positions) {
        const road = updates.roads.find(r => r.id === pos.roadId);
        if (!road)
 ...
 */
function forceMaxRecruitment(faction, locations, armies, currentGold) {
    ...
    for (const locId of homeLocations) {
        ...
        for (let i = 0; i < remainingRecruits; i++) {
            if (currentGold < constants_1.RECRUIT_COST)
                break;
            if (loc.population < constants_1.RECRUIT_AMOUNT)
                break;
 ...
 */
function deployTurn1Garrisons(state, faction, updates, currentGold) {
    const positions = [DEFENSIVE_POSITIONS.HEATHERFIELD, DEFENSIVE_POSITIONS.LIGHTHOUSE];
    const fortLevel1 = constants_1.FORTIFICATION_LEVELS[1]; // Pikes and Trenches
    for (const pos of positions) {
        // Find source army at Sunbreach Lands with enough soldiers
        const sourceArmy = updates.armies.find(a => a.faction === faction &&
 ...
 */
function deployTurn2Reinforcements(state, faction, updates, currentGold) {
    // Check if situation is favorable:
    // - No enemy armies in Sunbreach Lands
    // - Both defensive positions are still ours
    const enemiesInHome = state.armies.some(a => a.faction !== faction &&
        a.faction !== types_1.FactionId.NEUTRAL &&
        a.locationId === 'sunbreach_lands');
    if (enemiesInHome) {
    ...
    const positions = [DEFENSIVE_POSITIONS.HEATHERFIELD, DEFENSIVE_POSITIONS.LIGHTHOUSE];
    const fortLevel2 = constants_1.FORTIFICATION_LEVELS[2]; // Stone Tower
    for (const pos of positions) {
        const road = updates.roads.find(r => r.id === pos.roadId);
        if (!road)
            continue;
        ...
        if (stage.faction !== faction)
            continue;
        if ((stage.fortificationLevel || 0) < 1)
            continue;
        if (stage.activeConstruction)
        ...
        if (!sourceArmy)
            continue;
        if (currentGold < fortLevel2.cost)
            continue;
        // Find existing garrison at this position
        const existingGarrison = updates.armies.find(a => a.faction === faction &&
            a.roadId === pos.roadId &&
            a.stageIndex === pos.stageIndex &&
            a.locationType === 'ROAD');
        // Reinforce
