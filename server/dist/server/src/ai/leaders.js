...
exports.manageLeaders = void 0;
const types_1 = require("../../../shared/types");
const leaders_config_1 = require("./leaders_config");
const utils_1 = require("./utils");
const manageLeaders = (state, faction) => {
    let characters = [...state.characters];
    let logs = [];
    const myLeaders = characters.filter(c => c.faction === faction && c.status === types_1.CharacterStatus.AVAILABLE);
    const activeMissions = state.aiState?.[faction]?.missions || [];
    for (const leader of myLeaders) {
        const profile = (0, leaders_config_1.getLeaderProfile)(leader.name, leader.id);
        if (!profile)
            continue; // Skip generic leaders if any, or implement default logic
        // DECISION PROCESS:
        ...
        if (leader.locationId) {
            ...
            if (currentLoc && currentLoc.faction !== faction) {
                ...
                if (actionTaken)
                    continue; // Fleeing is the only priority
            }
        }
        ...
        if (profile.isVIP && leader.locationId) {
            ...
            if (currentLoc && currentLoc.faction === faction) {
                ...
                if (isCommanderVip && linkedLoc && linkedLoc.faction !== faction) {
                    ...
                    const activeCampaign = state.aiState?.[faction]?.missions.some(m => m.type === 'CAMPAIGN' &&
...
exports.manageLeaders = manageLeaders;
// Helper to evaluate and execute a role
function evaluateAndAssignRole(role, leader, state, faction, missions, allCharacters, logs) {
    if (!leader.locationId)
        return false;
    switch (role) {
        case 'COMMANDER':
...
function handleCommanderRole(leader, state, faction, missions, allCharacters, logs) {
    ...
    for (const mission of combatMissions) {
        ...
        if (!targetLocationId)
            continue;
        // Check reachability
        const path = (0, utils_1.findSafePath)(leader.locationId, targetLocationId, state, faction);
        if (!path)
            continue; // Cannot reach
        // Is there a significant army there (or en route) without a leader?
        const armyWithoutLeader = state.armies.find(a => a.faction === faction &&
        ...
        if (armyWithoutLeader) {
            ...
            if (leader.locationId === armyWithoutLeader.locationId) {
                ...
                if (idx !== -1) {
                    allCharacters[idx] = { ...leader, armyId: armyWithoutLeader.id, status: types_1.CharacterStatus.AVAILABLE }; // Occupied?
...
function handleManagerRole(leader, state, faction, allCharacters, logs) {
    if (leader.stats.ability.includes('MANAGER')) {
        ...
        for (const city of myCities) {
            const path = (0, utils_1.findSafePath)(leader.locationId, city.id, state, faction);
            if (path) {
...
function handleStabilizerRole(leader, state, faction, allCharacters, logs, isProtector = false, explicitTargetId = null) {
    ...
    if (explicitTargetId) {
        ...
        // Move there
        const path = (0, utils_1.findSafePath)(leader.locationId, explicitTargetId, state, faction);
    ...
    for (const mission of stabilizeMissions) {
        ...
        const otherStabilizer = allCharacters.find(c => c.locationId === targetId &&
            c.id !== leader.id &&
            c.faction === faction &&
            (c.status === types_1.CharacterStatus.AVAILABLE || c.status === types_1.CharacterStatus.MOVING) &&
        ...
        const path = (0, utils_1.findSafePath)(leader.locationId, targetId, state, faction);
...
// --- UTILS ---
function moveLeaderStart(leader, targetId, state, allCharacters, logs, reason) {
    const path = (0, utils_1.findSafePath)(leader.locationId, targetId, state, leader.faction);
...
function hasManager(locationId, chars, faction) {
    return chars.some(c => c.locationId === locationId && c.faction === faction && c.stats.ability.includes('MANAGER') && !c.armyId);
}
function hasStabilizer(locationId, chars, faction) {
    return chars.some(c => c.locationId === locationId && c.faction === faction && c.status === types_1.CharacterStatus.AVAILABLE && !c.armyId); // Any avail char acts as stabilizer? Or specific traits? 
