 ...
 */
function handleGrainEmbargo(state, faction, profile, locations) {
    const logs = [];
    let grainTradeNotification;
    ...
    else if (faction === types_1.FactionId.NOBLES) {
 ...
 */
function handleConspiratorEmbargo(state, windward, greatPlains, logs, setNotification) {
    if (windward.isGrainTradeActive) {
        // Check if Rivenberg is alive
        const rivenberg = state.characters.find(c => c.id === 'rivenberg' && c.status !== types_1.CharacterStatus.DEAD);
        if (!rivenberg)
 ...
 */
function handleNobleEmbargo(state, faction, locations, windward, greatPlains, logs, setNotification) {
    if (windward.isGrainTradeActive) {
        // Check if Baron Lekal OR Sir Haraldic are alive
        const lekal = state.characters.find(c => c.id === 'lekal' && c.status !== types_1.CharacterStatus.DEAD);
        const haraldic = state.characters.find(c => c.id === 'haraldic' && c.status !== types_1.CharacterStatus.DEAD);
        if (!lekal && !haraldic)
            return; // No political cover
 ...
 */
function isEmbargoAdvantageousForNobles(faction, locations) {
    const myLocations = locations.filter(l => l.faction === faction);
    const myCities = myLocations.filter(l => l.type === types_1.LocationType.CITY);
    const myRurals = myLocations.filter(l => l.type === types_1.LocationType.RURAL);
 ...
 */
function liftEmbargo(windward, greatPlains) {
