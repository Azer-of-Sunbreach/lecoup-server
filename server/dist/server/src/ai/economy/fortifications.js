...
exports.handleFortifications = handleFortifications;
 ...
 */
function handleFortifications(faction, locations, roads, armies, budget, profile, currentGold) {
    if (!profile.useFortifications)
        return currentGold;
    // Fortify locations
...
function fortifyLocations(faction, locations, armies, budget, currentGold) {
    const fortifiableLocs = locations.filter(l => l.faction === faction && !l.activeConstruction);
    for (const loc of fortifiableLocs) {
        const nextLevel = loc.fortificationLevel + 1;
        const data = constants_1.FORTIFICATION_LEVELS[nextLevel];
        const limit = loc.type === 'CITY'
            ? (loc.id === 'stormbay' ? 4 : 3)
            : 1;
        if (nextLevel <= limit && data && currentGold >= data.cost) {
            const garrison = armies.filter(a => a.locationId === loc.id &&
                a.faction === faction &&
                !a.action);
...
function fortifyRoadStages(faction, roads, armies, budget, currentGold) {
