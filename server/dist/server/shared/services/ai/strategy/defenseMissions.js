...
exports.generateDefendMissions = generateDefendMissions;
 ...
 */
function generateDefendMissions(state, faction, theaters, profile, activeMissions) {
    // Find vulnerable locations
    const vulnerableLocs = state.locations.filter(l => l.faction === faction &&
        (l.stability < 50 || (l.stability < 70 && l.type === 'CITY')));
    const locationsToDefend = new Set();
    // Collect locations needing defense
    for (const theater of theaters) {
        // Vital cities
        theater.locationIds
            .filter(id => types_1.VITAL_CITIES.includes(id))
    ...
    for (const locId of locationsToDefend) {
        ...
        if (activeMissions.some(m => m.type === 'DEFEND' && m.targetId === locId))
            continue;
