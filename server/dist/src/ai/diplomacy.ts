...
import { GameState, FactionId, CharacterStatus, Character, LocationType, AIMission } from '../../../shared/types';
...
export const manageDiplomacy = (
    ...
    const diploMissions = missions.filter(m =>
        ((m.type === 'INSURRECTION' && !disableInsurrections) || m.type === 'NEGOTIATE') &&
...
function handleInsurrection(
    ...
    if (currentGold < COST_INCITE) return currentGold;
    ...
    if (targetStability <= 30) baseSpend = Math.min(baseSpend, 200);
    else if (targetStability <= 50) baseSpend = Math.min(baseSpend, 300);
...
function handleNegotiation(
    ...
    const nearbyCities = updates.locations!.filter(l =>
        l.faction === faction &&
        l.type === LocationType.CITY &&

        l.foodStock > 150 && // User Threshold: 150 (Restored)
        l.foodIncome > 0 &&  // Must have positive flow (Restored)
        getDistance(l.id, mission.targetId, state.roads) <= 3 // Close enough for logistics
    );
