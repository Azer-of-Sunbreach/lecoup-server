...
const resolveMovements = (state) => {
    ...
    nextArmies.forEach(armyA => {
        ...
        if (moveA) {
            posNextA = getPositionKey(moveA.isMoveFinished ? 'LOCATION' : 'ROAD', moveA.nextLocationId, moveA.nextRoadId, moveA.nextStageIndex);
        }
        nextArmies.forEach(armyB => {
            if (armyA.id === armyB.id)
                return;
            if (!areHostile(armyA.faction, armyB.faction))
                return;
