...
exports.processClandestineAgent = processClandestineAgent;
 ...
 */
function processClandestineAgent(leader, locations, armies, turn, actorFaction, logs, allCharacters = [], roads = []) {
    ...
    let budget = leader.clandestineBudget ?? leader.budget ?? 0;
    ...
    let newPlannedMissionAction = leader.plannedMissionAction;
    ...
    for (const action of currentActions) {
        let cost = 0;
        if (action.actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS)
            cost = 50;
        if (action.actionId === clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES)
            cost = 20;
        if (action.actionId === clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS)
            cost = 10;
        if (action.actionId === clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA)
            cost = 10;
    ...
    const governorHere = characters.find(c => c.locationId === location.id &&
    ...
    const legendaryEnemy = characters.find(c => c.locationId === location.id &&
        c.faction === location.faction &&
    ...
    // =========================================================================
    const strategy = (0, factionStrategy_1.getStrategyForFaction)(actorFaction);
    const isAlreadyPreparingGI = currentActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION);
    const isPreparingGI = isAlreadyPreparingGI || leader.plannedMissionAction === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION;
    // Check if currently doing INCITE_NEUTRAL_INSURRECTIONS (gets +15% extra risk tolerance)
    const isDoingINCITE = currentActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS) || leader.plannedMissionAction === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS;
    // Perform standardized risk assessment
    const riskAssessment = (0, AIRiskDecisionService_1.assessRisk)({
        leader,
        location,
        governor: characters.find(c => c.locationId === location.id &&
            c.faction === location.faction &&
            c.status === types_1.CharacterStatus.GOVERNING),
        strategy,
    ...
    if (clandestineOps >= 3 && !hasLegendaryEnemy) {
        ...
        const insurgentsPerTurn = Math.floor((location.population * clandestineOps * (enemyResentment + 1)) /
    ...
    let bestInsurgentAction = actionScores.find(a => a.insurgentsPerGold > 0 &&
    ...
    const plannedActionId = leader.plannedMissionAction; // Cast as any if type not updated yet
    // DEBUG LOG REMOVED
    if (plannedActionId) {
        ...
        const hasMajorActionRef = currentActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS ||
            a.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION);
        ...
        if (hasMajorActionRef && !isScorchedEarth && bestInsurgentAction &&
            (bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS ||
                bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION)) {
    ...
    if (bestInsurgentAction) {
        ...
        if (remainingBudget >= totalCost) {
            ...
            if (bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION) {
            ...
            newActions.push({
                ...
                turnStarted: bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION
                    ? undefined // Let processor initialize and create prep log
                    : turn,
                oneTimeGoldAmount: bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION
                    ? giGoldAmount
                    : undefined
            });
            if (bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION) {
        ...
        else if (isPreparingGI && bestInsurgentAction.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION) {
    ...
    const hasIncite = newActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS);
    const hasGI = newActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION);
    ...
    if (isMinorMission && hasIncite) {
        ...
        const supportActions = [
            {
                id: clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES,
                cost: 10,
                detection: 10,
                blocked: () => location.stability <= 0
            },
            {
                id: clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS,
                cost: 10,
                detection: 5,
                blocked: () => getResentment(location, location.faction) >= 100
            },
            {
                id: clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA,
                cost: 10,
                detection: 5,
        ...
        for (const support of supportActions) {
            ...
            if (remainingBudget < support.cost)
                continue;
    ...
    const ACTION_PRIORITY = [
        ...
        clandestineTypes_1.ClandestineActionId.ATTACK_TAX_CONVOYS,
        clandestineTypes_1.ClandestineActionId.STEAL_FROM_GRANARIES,
        clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS,
        clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION
    ];
    ...
    if (projectedDetection > maxAllowedDetection || (riskCheckApplies && projectedCaptureRisk > loadSheddingMaxRisk)) {
        ...
        for (const actionId of ACTION_PRIORITY) {
            ...
            if (actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION)
                break;
            if (actionId === clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS)
    ...
    const justStartedGI = newActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION);
    ...
    if (shouldExfiltrate) {
        ...
        if (remainingBudget >= 100) {
            ...
            if (location.linkedLocationId)
                forbiddenIds.push(location.linkedLocationId);
            // 1. Identify candidates
            const candidates = locations.filter(l => !forbiddenIds.includes(l.id) &&
                l.faction !== actorFaction // Not friendly
            );
            // 2. Calculate travel times and sort
            const validTargets = [];
            for (const candidate of candidates) {
                const dist = (0, leaderPathfinding_1.calculateLeaderTravelTime)(location.id, candidate.id, locations, roads);
            ...
            if (validTargets.length > 0) {
                ...
                if (remainingBudget > 200) {
                    // Major Mission (> 200g)
                    newPlannedMissionAction = clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS;
                    ...
                    newPlannedMissionAction = clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES;
        ...
        if (!bestTargetId) {
            ...
            if (friendlyLocations.length > 0) {
                ...
                for (const friendly of friendlyLocations) {
                    const time = (0, leaderPathfinding_1.calculateLeaderTravelTime)(location.id, friendly.id, locations, roads);
                    if (time < minTime) {
                        minTime = time;
        ...
        if (bestTargetId) {
            ...
            const travelTime = (0, leaderPathfinding_1.calculateLeaderTravelTime)(location.id, bestTargetId, locations, roads);
