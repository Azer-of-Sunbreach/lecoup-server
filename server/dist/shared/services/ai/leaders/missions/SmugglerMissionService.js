...
exports.evaluateSmugglerDispatch = evaluateSmugglerDispatch;
exports.shouldCancelSmugglerMission = shouldCancelSmugglerMission;
exports.assignSmugglerMission = assignSmugglerMission;
exports.clearSmugglerMission = clearSmugglerMission;
 ...
 */
function evaluateSmugglerDispatch(context) {
    ...
    const smugglerAlreadyPresent = state.characters.some(c => c.faction === faction &&
 ...
 */
function findBestSmugglerCandidate(context) {
    ...
    for (const leader of state.characters) {
        ...
        if (!validStatuses.includes(leader.status)) {
        ...
        const travelTime = (0, leaderPathfinding_1.calculateLeaderTravelTime)(leader.locationId || '', cityId, state.locations, state.roads);
        // Skip if too far
        if (travelTime > MAX_SMUGGLER_TRAVEL_TIME)
            continue;
        const currentLoc = state.locations.find(l => l.id === leader.locationId);
        candidates.push({
            leader,
            ...
            isGoverning: leader.status === types_1.CharacterStatus.GOVERNING,
 ...
 */
function calculateFamineTimeline(city) {
    // Get food stock (assume a getter exists or use a default)
    const foodStock = city.foodStock ?? 50;
 ...
 */
function shouldCancelSmugglerMission(leader, state) {
    // Check if leader has an active smuggler mission
    const smugglerTargetCityId = leader.smugglerTargetCityId;
    if (!smugglerTargetCityId)
        return false;
    const targetCity = state.locations.find(l => l.id === smugglerTargetCityId);
    if (!targetCity)
        return true; // City no longer exists
 ...
 */
function assignSmugglerMission(leader, targetCityId) {
    return {
        ...leader,
 ...
 */
function clearSmugglerMission(leader) {
    const updated = { ...leader };
    delete updated.isSmugglerMission;
    delete updated.smugglerTargetCityId;
    return updated;
}
