...
exports.PRIORITY_LEADERS = exports.NOBLES_ABILITY_VALUES = exports.FIEF_COST_PER_TURN = exports.ASSUMED_REMAINING_TURNS = void 0;
exports.evaluateNoblesRecruitableLeader = evaluateNoblesRecruitableLeader;
exports.evaluateAllNoblesRecruitableLeaders = evaluateAllNoblesRecruitableLeaders;
exports.getBestNoblesRecruitableLeader = getBestNoblesRecruitableLeader;
exports.filterNoblesRecruitableLeaders = filterNoblesRecruitableLeaders;
 ...
 */
function evaluateNoblesRecruitableLeader(leader, context) {
    const reasoning = [];
 ...
 */
function calculateNoblesStabilityValue(leader, context, reasoning) {
 ...
 */
function calculateNoblesRoleValue(leader, context, reasoning) {
    ...
    const enemyLocations = context.locations.filter(l => l.faction !== types_1.FactionId.NOBLES && l.faction !== types_1.FactionId.NEUTRAL);
    ...
    if (clandestineOps > 0 && enemyLocations.length > 0) {
        ...
        const targetLocation = enemyLocations
            ...
            || enemyLocations[0];
        // Minor mission IPG
        const minorResult = (0, IPGCalculator_1.calculateMinorIPG)(targetLocation, clandestineOps, types_1.FactionId.NOBLES, discretion, types_1.FactionId.NOBLES);
        ...
        if (context.factionRevenues >= MIN_REVENUES_FOR_MAJOR_MISSIONS) {
            // INCITE_NEUTRAL IPG
            const neutralIPG = (0, IPGCalculator_1.calculateNeutralIPG)(targetLocation, clandestineOps, types_1.FactionId.NOBLES, discretion);
            ...
            const grandIPG = (0, IPGCalculator_1.calculateGrandIPG)(targetLocation, clandestineOps, 400, types_1.FactionId.NOBLES);
    ...
    if (territoriesNeedingStability.length > 0 && statesmanship > 0) {
        ...
        const govResult = (0, IPGCalculator_1.calculateGovernorIPG)(leader, targetTerritory, types_1.FactionId.NOBLES);
 ...
 */
function calculateNoblesAbilityBonuses(leader, context, reasoning) {
    ...
    const activeAbilities = [...abilities, ...grantedAbilities]
        .filter((a) => a != null && a !== 'NONE' && !disabledAbilities.includes(a));
    let totalBonus = 0;
    const bonusDetails = [];
    for (const ability of activeAbilities) {
        const valueOrFn = exports.NOBLES_ABILITY_VALUES[ability];
 ...
 */
function calculateSpecialEffectValue(leader, context, reasoning) {
    switch (leader.id) {
        case 'baron_ystrir': {
            // 550 gold immediate => 550/30 = 18.3g/turn equivalent
            const value = noblesRecruitment_1.BARON_YSTRIR_GOLD / exports.ASSUMED_REMAINING_TURNS;
            reasoning.push(`Special: +${value.toFixed(1)} (${noblesRecruitment_1.BARON_YSTRIR_GOLD}g immediate / ${exports.ASSUMED_REMAINING_TURNS} turns)`);
            return value;
        }
        case 'duke_esmarch': {
            // 2000 soldiers = 200g equivalent (10 soldiers per gold) => 200/30 = 6.7g/turn
            const goldEquivalent = noblesRecruitment_1.DUKE_ESMARCH_SOLDIERS / 10;
            const value = goldEquivalent / exports.ASSUMED_REMAINING_TURNS;
            reasoning.push(`Special: +${value.toFixed(1)} (${noblesRecruitment_1.DUKE_ESMARCH_SOLDIERS} soldiers = ${goldEquivalent}g / ${exports.ASSUMED_REMAINING_TURNS} turns)`);
            return value;
        }
        case 'duke_great_plains': {
            const windward = context.locations.find(l => l.id === 'windward');
            if (windward?.faction === types_1.FactionId.NOBLES) {
                // Windward controlled: +400g => 13.3g/turn
                const value = noblesRecruitment_1.DUKE_GREAT_PLAINS_BUDGET / exports.ASSUMED_REMAINING_TURNS;
                reasoning.push(`Special: +${value.toFixed(1)} (Windward controlled: ${noblesRecruitment_1.DUKE_GREAT_PLAINS_BUDGET}g / ${exports.ASSUMED_REMAINING_TURNS} turns)`);
                return value;
            }
            else {
                ...
                const discretion = getStatLevel(leader.stats?.discretion);
                if (windward) {
                    const grandIPG = (0, IPGCalculator_1.calculateGrandIPG)(windward, ops, noblesRecruitment_1.DUKE_GREAT_PLAINS_BUDGET, types_1.FactionId.NOBLES);
                    const neutralIPG = (0, IPGCalculator_1.calculateNeutralIPG)(windward, ops, types_1.FactionId.NOBLES, discretion);
                    const bestIPG = Math.max(grandIPG, neutralIPG);
                    reasoning.push(`Special: +${bestIPG.toFixed(1)} (Windward enemy: best IPG with ${noblesRecruitment_1.DUKE_GREAT_PLAINS_BUDGET}g budget)`);
                    return bestIPG;
                }
        ...
        case 'georges_cadal': {
            ...
            const targetTerritories = noblesRecruitment_1.GEORGES_CADAL_TERRITORIES
                .map(id => context.locations.find(l => l.id === id))
                .filter((l) => l !== undefined && l.faction !== types_1.FactionId.NOBLES);
            ...
            for (const territory of targetTerritories) {
                const insurgents = (0, IPGCalculator_1.calculateGrandInsurgents)(territory, ops, noblesRecruitment_1.GEORGES_CADAL_BUDGET, types_1.FactionId.NOBLES);
            ...
            const value = goldEquivalent / exports.ASSUMED_REMAINING_TURNS;
            reasoning.push(`Special: +${value.toFixed(1)} (${bestInsurgents} insurgents @ ${bestTerritory} = ${goldEquivalent.toFixed(0)}g / ${exports.ASSUMED_REMAINING_TURNS} turns)`);
            return value;
        }
        case 'duke_hornvale': {
            // Similar to georges_cadal but with different territories and budget
            const targetTerritories = noblesRecruitment_1.DUKE_HORNVALE_TERRITORIES
                .map(id => context.locations.find(l => l.id === id))
                .filter((l) => l !== undefined && l.faction !== types_1.FactionId.NOBLES);
            if (targetTerritories.length === 0) {
            ...
            const ops = getStatLevel(leader.stats?.clandestineOps);
            for (const territory of targetTerritories) {
                const insurgents = (0, IPGCalculator_1.calculateGrandInsurgents)(territory, ops, noblesRecruitment_1.DUKE_HORNVALE_BUDGET, types_1.FactionId.NOBLES);
                if (insurgents > bestInsurgents) {
                    bestInsurgents = insurgents;
                    bestTerritory = territory.name;
                }
 ...
 */
function evaluateAllNoblesRecruitableLeaders(recruitableLeaders, context) {
    const results = recruitableLeaders.map(leader => evaluateNoblesRecruitableLeader(leader, context));
 ...
 */
function getBestNoblesRecruitableLeader(recruitableLeaders, context) {
 ...
 */
function filterNoblesRecruitableLeaders(recruitableLeaders, context) {
    const shouldRestrictToSpecial = context.controlledLocations.length <= context.livingLeadersCount ||
        (context.factionRevenues < MIN_REVENUES_FOR_MAJOR_MISSIONS && context.foodSurplus < 50);
    if (shouldRestrictToSpecial) {
        return recruitableLeaders.filter(l => exports.PRIORITY_LEADERS.includes(l.id));
    }
