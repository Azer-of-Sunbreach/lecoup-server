...
exports.makeClandestineDecisions = makeClandestineDecisions;
exports.evaluateClandestineOpportunity = evaluateClandestineOpportunity;
...
const AIRiskDecisionService_1 = require("../utils/AIRiskDecisionService");
// ============================================================================
 ...
 */
function makeClandestineDecisions(leader, location, faction, strategy, budget, turn, armies, characters, opportunity) {
    ...
    const isPreparingInsurrection = activeActions.some(a => a.actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION);
    ...
    const riskAssessment = (0, AIRiskDecisionService_1.assessRisk)(riskContext);
    // Calculate legacy risk for backward compatibility (deprecated, will be removed)
    const currentRisk = (0, detectionLevelService_1.calculateCaptureRisk)(leader, location, governor) / 100;
    const context = {
        leader,
        location,
        faction,
    ...
    const reasoning = [];
    const actionsToStart = [];
    const actionsToStop = [];
    let goldToAllocate;
    // Log current risk state
    reasoning.push(`Risk: ${(0, AIRiskDecisionService_1.formatRiskSummary)(riskAssessment)}`);
    ...
    const exfiltrationDecision = (0, AIRiskDecisionService_1.shouldExfiltrateLeader)(riskContext, budget, canPerformAnyAction);
    if (exfiltrationDecision.shouldExfiltrate) {
        return {
            leaderId: leader.id,
            targetLocationId: location.id,
            actionsToStart: [],
            actionsToStop: activeActions.map(a => a.actionId),
            shouldExfiltrate: true,
            shouldGoDark: false,
            reasoning: [
                `EXFILTRATE: ${exfiltrationDecision.reason || 'Risk too high'}`,
                `${(0, AIRiskDecisionService_1.formatRiskSummary)(riskAssessment)}`
            ]
        };
    }
    ...
    const selectedActions = selectOptimalActionsWithDetection(context, actionScores, mandatoryActions, riskAssessment);
    // 3. Determine changes
    ...
    for (const actionId of selectedActions) {
        if (!currentActionIds.has(actionId)) {
            ...
            if (actionId === clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION) {
                goldToAllocate = Math.min(budget, types_2.CLANDESTINE_BUDGET.MAX);
            }
        }
    }
 ...
 */
function getMandatoryActions(leader, locationType) {
    const mandatory = [];
    // SCORCHED_EARTH forces destructive actions
    if (leader.stats?.traits?.includes('SCORCHED_EARTH')) {
        if (locationType === 'CITY') {
            mandatory.push(clandestineTypes_1.ClandestineActionId.START_URBAN_FIRE);
        }
        else {
            mandatory.push(clandestineTypes_1.ClandestineActionId.BURN_CROP_FIELDS);
        }
        mandatory.push(clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS);
    }
    return mandatory;
}
/**
 ...
 */
function getBlockedActions(leader) {
    const blocked = [];
    // FAINT_HEARTED blocks violent actions
    if (leader.stats?.traits?.includes('FAINT_HEARTED')) {
        blocked.push(clandestineTypes_1.ClandestineActionId.ASSASSINATE_LEADER);
        blocked.push(clandestineTypes_1.ClandestineActionId.BURN_CROP_FIELDS);
        blocked.push(clandestineTypes_1.ClandestineActionId.START_URBAN_FIRE);
    }
    return blocked;
}
// ============================================================================
 ...
 */
function scoreAllActions(context, opportunity, mandatoryActions, blockedActions) {
    const scores = [];
    ...
    // =========================================================================
    {
        ...
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION)) {
            score = -1000;
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION,
            score,
            reasoning,
            costPerTurn: 0,
            oneTimeCost: cost,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.PREPARE_GRAND_INSURRECTION] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        const cost = clandestineTypes_1.CLANDESTINE_ACTION_COSTS[clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES] || 20;
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
        else {
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES,
            score,
            reasoning,
            costPerTurn: cost,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.UNDERMINE_AUTHORITIES] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        const cost = clandestineTypes_1.CLANDESTINE_ACTION_COSTS[clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS] || 10;
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
        else {
            // Lower resentment against us = more valuable
            const ourResentment = location.resentment?.[context.faction] || 50;
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS,
            score,
            reasoning,
            costPerTurn: cost,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.DISTRIBUTE_PAMPHLETS] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        const cost = clandestineTypes_1.CLANDESTINE_ACTION_COSTS[clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA] || 10;
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
            ...
            const enemyResentment = location.resentment?.[location.faction] || 50;
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA,
            score,
            reasoning,
            costPerTurn: cost,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.SPREAD_PROPAGANDA] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        const cost = clandestineTypes_1.CLANDESTINE_ACTION_COSTS[clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS] || 50;
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
        ...
        else if (mandatoryActions.includes(clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS)) {
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS,
            score,
            reasoning,
            costPerTurn: cost,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.INCITE_NEUTRAL_INSURRECTIONS] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.ATTACK_TAX_CONVOYS)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
        else if (clandestineOps < 2) {
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.ATTACK_TAX_CONVOYS,
            score,
            reasoning,
            costPerTurn: 0,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.ATTACK_TAX_CONVOYS] * 0.1
        });
    }
    // =========================================================================
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        if (blockedActions.includes(clandestineTypes_1.ClandestineActionId.STEAL_FROM_GRANARIES)) {
            score = -1000;
            reasoning = 'Blocked by trait';
        }
        else if (clandestineOps < 2) {
        ...
        scores.push({
            actionId: clandestineTypes_1.ClandestineActionId.STEAL_FROM_GRANARIES,
            score,
            reasoning,
            costPerTurn: 0,
            oneTimeCost: 0,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[clandestineTypes_1.ClandestineActionId.STEAL_FROM_GRANARIES] * 0.1
        });
    }
    // =========================================================================
        ...
        : clandestineTypes_1.ClandestineActionId.BURN_CROP_FIELDS;
    {
        let score = 0;
        let reasoning = '';
        const cost = clandestineTypes_1.CLANDESTINE_ACTION_COSTS[burnAction] || 25;
        ...
        scores.push({
            ...
            costPerTurn: 0,
            oneTimeCost: cost,
            addedRisk: clandestineTypes_1.CLANDESTINE_ACTION_RISKS[burnAction] * 0.1
        });
    }
 ...
 */
function selectOptimalActions(context, actionScores, mandatoryActions) {
    const selected = [...mandatoryActions];
    ...
    const currentActionIds = context.activeActions.map(a => a.actionId);
    ...
    for (const scoreInfo of actionScores) {
        if (scoreInfo.score <= 0)
            continue;
        if (selected.includes(scoreInfo.actionId))
            continue;
        // Budget check
 ...
 */
function selectOptimalActionsWithDetection(context, actionScores, mandatoryActions, riskAssessment) {
    const selected = [];
    let remainingBudget = context.budget;
    ...
    for (const action of context.activeActions) {
        ...
        if (projectedRisk > maxCaptureRisk)
            continue;
    ...
    for (const scoreInfo of actionScores) {
        ...
        if (selected.includes(scoreInfo.actionId))
            continue;
        const actionDef = clandestineTypes_1.CLANDESTINE_ACTIONS[scoreInfo.actionId];
        if (!actionDef)
            continue;
 ...
 */
function evaluateClandestineOpportunity(location, leader, armies, characters, faction) {
    ...
    const ownerResentment = location.resentment?.[location.faction] || 50;
    const actorResentment = location.resentment?.[faction] || 50;
