...
exports.makeGovernorDecisions = makeGovernorDecisions;
exports.analyzeTerritoryForGovernor = analyzeTerritoryForGovernor;
 ...
 */
function makeGovernorDecisions(leader, location, territory, strategy, availableGold, availableFood, turn) {
    const context = {
        leader,
        location,
        territory,
    ...
    const selectedPolicies = selectOptimalPolicies(context, policyScores, mandatoryPolicies, currentPolicies);
    // 4. Determine which policies to activate/deactivate
    const policiesToActivate = [];
    const policiesToDeactivate = [];
 ...
 */
function getMandatoryPolicies(leader) {
    const mandatory = [];
    // IRON_FIST trait forces MAKE_EXAMPLES
    if (leader.stats?.traits?.includes('IRON_FIST')) {
        mandatory.push(governorTypes_1.GovernorPolicy.MAKE_EXAMPLES);
    }
    return mandatory;
}
/**
 ...
 */
function scoreAllPolicies(context, currentPolicies) {
    const scores = [];
    ...
    // =========================================================================
    {
        ...
        else if (territory.stability < types_2.STABILITY_THRESHOLDS.LOW) {
            score = 80;
            reasoning = 'Low stability - high priority';
        }
        else if (territory.stability < types_2.STABILITY_THRESHOLDS.STABLE) {
            score = 50;
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.STABILIZE_REGION,
            score,
            reasoning,
    ...
    // =========================================================================
    {
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.APPEASE_MINDS,
            score,
            reasoning,
            goldCost: 0,
    ...
    // =========================================================================
    {
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.DENOUNCE_ENEMIES,
            score,
            reasoning,
            goldCost,
            foodCost: 0,
    ...
    // =========================================================================
    {
        ...
        if (strategy.preferMakeExamples) {
            ...
            if (territory.stability < types_2.STABILITY_THRESHOLDS.LOW) {
                score = 60;
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.MAKE_EXAMPLES,
            score,
            reasoning,
            goldCost: 0,
            foodCost: 0,
    ...
    // =========================================================================
    {
        let score = 0;
        let reasoning = '';
        const goldCost = governorTypes_1.GOVERNOR_POLICY_COSTS[governorTypes_1.GovernorPolicy.HUNT_NETWORKS] || 20;
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.HUNT_NETWORKS,
            score,
            reasoning,
            goldCost,
            foodCost: 0,
    ...
    // =========================================================================
    {
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.IMPROVE_ECONOMY,
            score,
            reasoning,
            goldCost: 0,
            foodCost: 0,
    ...
    if (isCity) {
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.RATIONING,
            score,
            reasoning,
            goldCost: 0,
            foodCost: 0,
    ...
    // =========================================================================
    {
        ...
        scores.push({
            policy: governorTypes_1.GovernorPolicy.REBUILD_REGION,
            score,
            reasoning,
            goldCost,
            foodCost: 0,
 ...
 */
function selectOptimalPolicies(context, policyScores, mandatoryPolicies, currentPolicies) {
    const selected = [...mandatoryPolicies];
    ...
    for (const policy of currentPolicies) {
        if (governorTypes_1.FULL_TIME_POLICIES.includes(policy)) {
            const scoreInfo = policyScores.find(p => p.policy === policy);
    ...
    for (const scoreInfo of policyScores) {
        if (scoreInfo.score <= 0)
            continue;
        if (selected.includes(scoreInfo.policy))
            continue;
        // Check full-time constraint
        if (scoreInfo.isFullTime) {
            if (hasFullTime)
                continue; // Already have a full-time policy
        }
        // Check cost constraints
        if (scoreInfo.goldCost > remainingGold)
            continue;
        if (scoreInfo.foodCost > remainingFood)
            continue;
        // Add the policy
 ...
 */
function calculateAppeaseMindsFood(population) {
    if (population < 25000)
        return 1;
    if (population <= 70000)
        return 2;
 ...
 */
function estimateStabilityGain(policies, leader, location) {
    let gain = 0;
    const statesmanship = leader.stats?.statesmanship || 3;
    // Passive stability from leader
    gain += leader.stats?.stabilityPerTurn || 0;
    // Policy effects
    if (policies.includes(governorTypes_1.GovernorPolicy.STABILIZE_REGION)) {
    ...
    if (policies.includes(governorTypes_1.GovernorPolicy.MAKE_EXAMPLES)) {
        gain += 1; // Small immediate benefit but long-term resentment cost
    }
    // RATIONING hurts stability
    if (policies.includes(governorTypes_1.GovernorPolicy.RATIONING)) {
        gain -= 2;
    }
    return gain;
}
 ...
 */
function analyzeTerritoryForGovernor(location, characters, faction, garrisonStrength, logs) {
    ...
    const enemyAgents = characters.filter(c => c.faction !== faction &&
    ...
    const enemyAgentConfidence = hasEnemyAgent ? 'CONFIRMED' : 'NONE';
    ...
    const hasGovernor = characters.some(c => c.faction === faction &&
        c.locationId === location.id &&
        c.status === 'GOVERNING');
    // Check if needs protector
    const hasLegendary = characters.some(c => c.faction === faction &&
        c.locationId === location.id &&
        c.stats?.ability?.includes('LEGENDARY'));
    // Check if needs stabilizer
    const hasStabilizer = characters.some(c => c.faction === faction &&
        c.locationId === location.id &&
 ...
 */
function isClandestineLog(message) {
    const keywords = [
