...
exports.ENABLE_INTERNAL_FACTION_LOGS = exports.INTERNAL_FACTION_MIN_TURN = void 0;
exports.checkRabbleVictoryConditions = checkRabbleVictoryConditions;
exports.checkKnightlyCoupConditions = checkKnightlyCoupConditions;
exports.checkMerchantDominationConditions = checkMerchantDominationConditions;
exports.processRepublicanInternalFaction = processRepublicanInternalFaction;
exports.applyInternalFactionResult = applyInternalFactionResult;
const types_1 = require("../../../../types");
const internalFactions_1 = require("../../../domain/internalFactions/internalFactions");
const characters_1 = require("../../../../data/characters");
const RabbleVictoryInsurrectionService_1 = require("./RabbleVictoryInsurrectionService");
// ============================================================================
// CONSTANTS
// ============================================================================
/** Turn at which internal faction decision becomes available */
exports.INTERNAL_FACTION_MIN_TURN = 6;
...
exports.ENABLE_INTERNAL_FACTION_LOGS = true;
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
/**
 ...
 */
function calculateFactionRevenue(locations, faction) {
    return locations
 ...
 */
function hasCompleteCityRuralPair(locations, faction) {
 ...
 */
function hasBesiegedCity(locations, faction) {
    const controlledSet = new Set(locations.filter(loc => loc.faction === faction).map(loc => loc.id));
    for (const loc of locations) {
        if (loc.faction === faction && loc.type === 'CITY' && loc.linkedLocationId) {
            if (!controlledSet.has(loc.linkedLocationId)) {
 ...
 */
function calculateLeaderStabilitySum(characters, leaderIds) {
    let sum = 0;
    for (const id of leaderIds) {
        const leader = characters.find(c => c.id === id && c.status !== types_1.CharacterStatus.DEAD);
        if (leader) {
            sum += (0, internalFactions_1.getEffectiveStabilityModifier)(leader);
        }
    }
    return sum;
}
 ...
 */
function getMerchantLeadersStabilityBonus() {
    let sum = 0;
    for (const id of MERCHANT_LEADERS) {
        const leaderData = characters_1.CHARACTERS_NEW.find(c => c.id === id);
        if (leaderData) {
 ...
 */
function checkRabbleVictoryConditions(state, faction) {
 ...
 */
function checkKnightlyCoupConditions(state, faction) {
 ...
 */
function checkMerchantDominationConditions(state, faction) {
    const controlled = getControlledLocations(state.locations, faction);
    const territoriesCount = controlled.length;
 ...
 */
function processRepublicanInternalFaction(state, faction, turn) {
    const logs = [];
    // Validate faction
    if (faction !== types_1.FactionId.REPUBLICANS) {
        return {
            choiceMade: false,
            chosenOption: null,
            goldCost: 0,
    ...
    if (turn < exports.INTERNAL_FACTION_MIN_TURN) {
        logs.push(`[AI INTERNAL FACTION] Turn ${turn} < ${exports.INTERNAL_FACTION_MIN_TURN} - not available yet`);
        return {
            choiceMade: false,
            chosenOption: null,
            goldCost: 0,
    ...
    const aiInternalState = state.aiState?.[types_1.FactionId.REPUBLICANS]?.republicanInternalFaction;
    ...
    if (rabbleCheck.conditionsMet) {
        ...
        const result = (0, internalFactions_1.executeRabbleVictoryChoice)(state.characters, state.locations, faction);
        if (result.success) {
            return {
                choiceMade: true,
    ...
    if (knightlyCheck.conditionsMet) {
        ...
        if (currentSavingsMode === 'KNIGHTLY_COUP' || knightlyCheck.canAffordNow) {
            ...
            if (totalAvailable >= INTERNAL_FACTION_COST) {
                ...
                const result = (0, internalFactions_1.executeKnightlyCoupChoice)(state.characters, state.locations, faction);
                if (result.success) {
                    return {
                        choiceMade: true,
    ...
    if (merchantCheck.conditionsMet) {
        ...
        if (currentSavingsMode === 'MERCHANT_DOMINATION' || merchantCheck.canAffordNow) {
            ...
            if (totalAvailable >= INTERNAL_FACTION_COST) {
                ...
                const result = (0, internalFactions_1.executeMerchantDominationChoice)(state.characters, state.locations, faction);
                if (result.success) {
                    return {
                        choiceMade: true,
                        ...
                        goldCost: internalFactions_1.MERCHANT_DOMINATION_GOLD_COST,
                        inSavingsMode: false,
                        savingsTarget: null,
                        savedGold: 0,
                        logs
 ...
 */
function applyInternalFactionResult(state, result) {
    let newState = { ...state };
    if (result.choiceMade && result.chosenOption) {
        ...
        if (result.chosenOption === 'RABBLE_VICTORY') {
            const dispatchResult = (0, RabbleVictoryInsurrectionService_1.dispatchRabbleVictoryLeaders)(newState, types_1.FactionId.REPUBLICANS);
            if (dispatchResult.success) {
                newState.characters = dispatchResult.updatedCharacters;
                // Add dispatch logs to result
                if (exports.ENABLE_INTERNAL_FACTION_LOGS) {
        ...
        if (result.goldCost > 0) {
            const currentGold = newState.resources[types_1.FactionId.REPUBLICANS]?.gold || 0;
            newState.resources = {
                ...newState.resources,
                [types_1.FactionId.REPUBLICANS]: {
                    ...newState.resources[types_1.FactionId.REPUBLICANS],
                    gold: currentGold - result.goldCost
                }
            };
        }
        // Clear savings mode
        const existingRepAiState = newState.aiState?.[types_1.FactionId.REPUBLICANS];
        newState.aiState = {
            ...newState.aiState,
            [types_1.FactionId.REPUBLICANS]: {
    ...
    // Log results
    if (exports.ENABLE_INTERNAL_FACTION_LOGS && result.logs.length > 0) {
