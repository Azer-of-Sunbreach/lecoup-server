...
exports.distributeClandestineBudget = distributeClandestineBudget;
/**
 ...
 */
function distributeClandestineBudget(state, faction, budget) {
    ...
    const scoredCandidates = updatedCharsPreNormalize.filter(c => c.faction === faction && !c.isDead)
        ...
        // Traits & Abilities
        if (stats?.ability?.includes('FIREBRAND'))
            score += 3; // Bonus for insurrection
        if (stats?.ability?.includes('DAREDEVIL'))
            score += 2; // Bonus for survival
        if (stats?.ability?.includes('GHOST'))
            score += 2; // Bonus for entry/exit
        if (stats?.traits?.includes('SCORCHED_EARTH'))
            score += 2; // Bonus for destructive potential
    ...
    for (const leader of candidates) {
        if (remainingTransferBudget < 100)
            break; // No viable budget left
        ...
        if (amountToGive < 100)
            break; // Should be handled by check above, but safety
        // Execute changes
