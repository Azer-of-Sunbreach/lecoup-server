...
exports.ROAD_DEFENSE_TARGETS = void 0;
exports.generateRoadDefenseMissions = generateRoadDefenseMissions;
const types_1 = require("../../../types");
exports.ROAD_DEFENSE_TARGETS = {
 ...
 */
function generateRoadDefenseMissions(state, faction, turn, activeMissions) {
    const targets = exports.ROAD_DEFENSE_TARGETS[faction] || [];
    if (targets.length === 0)
        return;
    ...
    const scoredTargets = targets.map(target => {
        ...
        if (!road)
            return { target, score: -1000 };
        const stageIndex = road.stages.findIndex(s => s.name?.toLowerCase().includes(target.stageId.toLowerCase()) ||
            s.id === target.stageId);
        if (stageIndex === -1)
            return { target, score: -1000 };
        ...
        if (stage.faction === faction) {
            ...
            const hasGarrison = state.armies.some(a => a.faction === faction &&
                a.locationType === 'ROAD' &&
                a.roadId === road.id &&
        ...
        const enemyOnRoad = state.armies.some(a => a.faction !== faction &&
    ...
    for (let i = 0; i < Math.min(maxNewMissions, scoredTargets.length); i++) {
        ...
        if (activeMissions.some(m => m.type === 'ROAD_DEFENSE' &&
