...
const freeTrader_1 = require("./domain/economy/freeTrader");
...
const processTurn = async (initialState, options = {}) => {
    ...
    try {
        ...
        const noblesAvailResult = (0, leaders_1.processNoblesLeaderAvailability)(state.characters, state.turn);
        ...
        state.locations = state.locations.map(loc => {
            const result = (0, freeTrader_1.enforceFreeTraderLimits)(loc, state.characters);
            if (result.modified) {
        ...
        // 5.3 Economy
        state.locations = (0, economy_1.calculateEconomyAndFood)(state, state.locations, state.armies, state.characters, state.roads);
        // 5.4 Famine
            ...
            if (ownedLocations.length === state.locations.length) {
                ...
                switch (faction) {
                    case types_1.FactionId.REPUBLICANS:
                        messageKey = "victory.messages.REPUBLICANS";
                        break;
                    case types_1.FactionId.CONSPIRATORS:
                        messageKey = "victory.messages.CONSPIRATORS";
                        break;
                    case types_1.FactionId.NOBLES:
                        messageKey = "victory.messages.NOBLES";
                        break;
                }
                state.victory = {
                    winner: faction,
