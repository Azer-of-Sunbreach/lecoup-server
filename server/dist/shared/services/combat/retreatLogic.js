...
exports.getRetreatPosition = void 0;
 ...
 */
const getRetreatPosition = (army, roads, locations) => {
    // Road retreat logic
    ...
    if (army.locationType === 'LOCATION' && army.locationId) {
            ...
            if (army.startOfTurnPosition.type === 'ROAD') {
                ...
                if (army.startOfTurnPosition.id === currentId) {
                    ...
                    if (army.tripOriginId && army.tripOriginId !== currentId) {
                        const road = roads.find(r => (r.from === currentId && r.to === army.tripOriginId) ||
                    ...
                    const road = roads.find(r => (r.from === currentId && r.to === targetId) ||
        ...
        let road = roads.find(r => (r.from === originId && r.to === currentId) ||
            (r.to === originId && r.from === currentId));
        // Fallback: search for ANY road connected to currentId
        if (!road) {
            const connectedRoads = roads.filter(r => r.from === currentId || r.to === currentId);
...
exports.getRetreatPosition = getRetreatPosition;
