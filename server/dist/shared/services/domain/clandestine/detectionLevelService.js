...
exports.calculateDetectionThreshold = calculateDetectionThreshold;
exports.getBaseDetectionThreshold = getBaseDetectionThreshold;
exports.calculateCaptureRisk = calculateCaptureRisk;
exports.isThresholdExceeded = isThresholdExceeded;
exports.calculateTurnDetectionIncrease = calculateTurnDetectionIncrease;
exports.applyOneTimeDetectionIncrease = applyOneTimeDetectionIncrease;
exports.applyTurnDetectionIncrease = applyTurnDetectionIncrease;
exports.getDetectionGaugeColor = getDetectionGaugeColor;
exports.getDetectionGaugeFill = getDetectionGaugeFill;
exports.shouldEffectsApply = shouldEffectsApply;
exports.markEffectsNotified = markEffectsNotified;
exports.clearPendingDetectionEffects = clearPendingDetectionEffects;
exports.shouldResetDetectionLevel = shouldResetDetectionLevel;
exports.applyDetectionLevelReset = applyDetectionLevelReset;
 ...
 */
function calculateDetectionThreshold(leader, location) {
    ...
    const isHuntNetworksActive = location.governorPolicies?.[governorTypes_1.GovernorPolicy.HUNT_NETWORKS] === true;
 ...
 */
function getBaseDetectionThreshold(leader) {
    const stealthLevel = leader.stealthLevel ?? leader.stats?.discretion ?? DEFAULT_STEALTH_LEVEL;
 ...
 */
function calculateCaptureRisk(leader, location, governor) {
 ...
 */
function isThresholdExceeded(leader, location) {
    const detectionLevel = leader.detectionLevel ?? 0;
    const threshold = calculateDetectionThreshold(leader, location);
 ...
 */
function calculateTurnDetectionIncrease(actions) {
    let totalIncrease = 0;
 ...
 */
function applyOneTimeDetectionIncrease(leader, actionId) {
 ...
 * @returns Updated leader with increased detection level
 */
function applyTurnDetectionIncrease(leader) {
 ...
 */
function getDetectionGaugeColor(detectionLevel, threshold) {
    if (detectionLevel === 0)
        return 'green';
 ...
 */
function getDetectionGaugeFill(detectionLevel, threshold) {
    if (threshold <= 0)
        return 100;
 ...
 */
function shouldEffectsApply(leader, isAIControlled) {
 ...
 */
function markEffectsNotified(leader, options) {
 ...
 */
function clearPendingDetectionEffects(leader) {
    return {
        ...leader,
 ...
 */
function shouldResetDetectionLevel(oldStatus, newStatus, oldLocationId, newLocationId) {
    const clandestineStatuses = [types_1.CharacterStatus.UNDERCOVER, types_1.CharacterStatus.ON_MISSION];
 ...
 */
function applyDetectionLevelReset(leader, oldStatus, newStatus, oldLocationId, newLocationId) {
