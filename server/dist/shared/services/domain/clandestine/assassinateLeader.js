...
exports.isAssassinationAvailable = isAssassinationAvailable;
exports.calculateAssassinationChance = calculateAssassinationChance;
exports.isTargetAccessible = isTargetAccessible;
 ...
 */
function isAssassinationAvailable(location, leader) {
    // 1. Check Trait
    ...
    const factionKey = controllerFaction !== types_1.FactionId.NEUTRAL ? controllerFaction : null;
 ...
 */
function calculateAssassinationChance(location, goldSpent, enemySoldiersCount) {
    const controllerFaction = location.faction;
    const factionKey = controllerFaction !== types_1.FactionId.NEUTRAL ? controllerFaction : null;
    const resentment = (factionKey && location.resentment?.[factionKey]) ?? 0;
    const stability = location.stability;
 ...
 */
function isTargetAccessible(targetId, assassinLocation, allCharacters, allLocations) {
    ...
    if (target.locationId === assassinLocation.id)
        return true;
