...
exports.calculateDetectionRisk = calculateDetectionRisk;
 ...
 */
function calculateDetectionRisk(location, activeActions, infiltratingLeader, allArmies, governor, isCounterEspionageActive = false) {
    ...
    const resentment = (0, resentment_1.getResentment)(location, location.faction); // Resentment against controller
    ...
    if (activeActions && activeActions.length > 0) {
        activeActions.forEach(action => {
            const riskLevel = clandestineTypes_1.CLANDESTINE_ACTION_RISKS[action.actionId] || 0;
