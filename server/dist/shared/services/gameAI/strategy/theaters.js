...
exports.analyzeTheaters = analyzeTheaters;
 ...
 */
function analyzeTheaters(state, faction) {
    ...
    for (const loc of ownedLocs) {
        ...
        while (queue.length > 0) {
            ...
            const armiesHere = state.armies.filter(a => a.locationId === currentId && a.faction === faction);
            strength += (0, utils_1.getArmyStrength)(armiesHere, state.characters);
            // Check connections
            ...
            for (const road of roads) {
                ...
                const enemiesOnRoad = state.armies.some(a => a.roadId === road.id && a.faction !== faction);
                if (enemiesOnRoad)
                    contested = true;
                if (neighbor && neighbor.faction === faction) {
                    if (!visited.has(neighborId))
                        queue.push(neighborId);
                    if (!internalRoads.includes(road.id))
                        internalRoads.push(road.id);
                }
                else if (neighbor) {
                    if (!borders.includes(neighborId))
                        borders.push(neighborId);
                    const enemyArmies = state.armies.filter(a => a.locationId === neighborId && a.faction !== faction);
                    threat += (0, utils_1.getArmyStrength)(enemyArmies, state.characters);
                }
            }
        }
        theaters.push({
