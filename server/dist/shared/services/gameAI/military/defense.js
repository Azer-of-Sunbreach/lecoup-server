...
exports.handleDefense = handleDefense;
...
function handleDefense(mission, state, faction, armies, assigned) {
    ...
    if (besiegerStr > 0 && currentStr > besiegerStr * 1.5) {
        ...
        atTarget.forEach(a => {
            ...
            if (idx !== -1) {
                armies[idx] = { ...a, isGarrisoned: false }; // Leave walls to fight
                assigned.add(a.id);
            }
        });
    ...
    for (const army of atTarget) {
        if (assignedStr < defenseCap) {
            ...
            // Excess troops - don't add to assigned, keep available for campaigns
            const idx = armies.findIndex(x => x.id === army.id);
            if (idx !== -1) {
                armies[idx] = { ...army, isGarrisoned: true };
            }
    ...
    if (currentStr < reqStrength) {
        (0, movement_1.pullReinforcements)(targetId, armies, state, faction, assigned, reqStrength - currentStr);
    }
 ...
 */
function deployScreenForces(targetId, state, faction, armies, atTarget, assigned, currentStr) {
    const roads = state.roads.filter(r => r.from === targetId || r.to === targetId);
    for (const road of roads) {
        // Check if we already have a screener
        const hasScreener = armies.some(a => a.faction === faction &&
            a.locationType === 'ROAD' &&
            a.roadId === road.id &&
            !assigned.has(a.id));
        if (!hasScreener) {
