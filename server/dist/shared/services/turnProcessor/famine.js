 ...
 */
function processFamine(state) {
    const logs = [];
    let locations = state.locations.map(l => ({ ...l }));
    let armies = [...state.armies];
    let stats = { ...state.stats };
    let famineNotification = null;
    const cityIdsInFamine = [];
    ...
    for (const cityId of cityIdsInFamine) {
        ...
        const cityArmies = armies.filter(a => a.locationId === cityId &&
            ...
            (a.faction === types_1.FactionId.REPUBLICANS ||
                a.faction === types_1.FactionId.CONSPIRATORS ||
                a.faction === types_1.FactionId.NOBLES));
        if (cityArmies.length > 0) {
            const armyDeaths = Math.floor(Math.random() * 2500) + 1;
            const { updatedArmies, deadArmyIds } = (0, combat_1.applySequentialLosses)(cityArmies, armyDeaths);
        ...
        if (city.linkedLocationId) {
            ...
            if (rural && rural.foodIncome <= 0) {
                ...
                if (ruralArmies.length > 0) {
                    ...
                    const { updatedArmies: rUpdated, deadArmyIds: rDead } = (0, combat_1.applySequentialLosses)(ruralArmies, rArmyDeaths);
