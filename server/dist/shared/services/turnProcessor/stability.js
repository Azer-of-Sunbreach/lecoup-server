 ...
 */
function applyLowTaxStabilityRecovery(locations) {
    const updatedLocations = locations.map(loc => {
        // Cities: Check personal tax level
        if (loc.type === types_1.LocationType.CITY && loc.taxLevel === 'VERY_LOW') {
            if (loc.stability < 25) {
        ...
        if (loc.type === types_1.LocationType.RURAL && loc.foodCollectionLevel === 'VERY_LOW') {
            if (loc.stability < 25) {
