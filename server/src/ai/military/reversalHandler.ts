import { GameState, FactionId, Army, LocationType } from '../../../../shared/types';
 ...
 */
export function handleEnRouteReversals(
    state: GameState,
    faction: FactionId,
    armies: Army[]
): void {
    const onRoadArmies = armies.filter(a =>
        a.faction === faction &&
        a.locationType === 'ROAD' &&
        a.roadId
    );
    ...
    for (const army of onRoadArmies) {
        ...
        if (!army.originLocationId) continue;
        ...
        if (originLoc.faction === faction || originLoc.faction === FactionId.NEUTRAL) {
        ...
        if (targetLoc.faction === faction) continue;
        ...
        if (army.strength < confidentAttackThreshold) {
            ...
            const result = executeRetreat(state, army.id);
 ...
 */
function calculateDefenseStrength(state: GameState, locationId: string, faction: FactionId): number {
    // 1. Enemy Armies (Garrison)
    const armies = state.armies.filter(a =>
