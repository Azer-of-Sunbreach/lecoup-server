...
import { GameState, FactionId, AIMission, Army, Road } from '../../../../shared/types';
 ...
 */
export function handleRoadDefense(
    mission: AIMission,
    state: GameState,
    faction: FactionId,
    armies: Army[],
    ...
    const stageIndex = road.stages.findIndex(s =>
        s.name?.toLowerCase().includes(stageId?.toLowerCase() || '')
    );
    if (stageIndex === -1) {
        mission.status = 'FAILED';
        return;
    ...
    if (mission.stage === 'GATHERING') {
        ...
        const armyAtStage = armies.find(a =>
            a.faction === faction &&
            a.roadId === roadId &&
        ...
        if (armyAtStage) {
            ...
            if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Army already at ${stageId}, garrisoning`);
            return;
        }

        // Find nearby army to send
        const candidateArmy = armies.find(a =>
            a.faction === faction &&
            !assigned.has(a.id) &&
        ...
        if (!candidateArmy) {
            ...
            const smallerArmy = armies.find(a =>
                a.faction === faction &&
                !assigned.has(a.id) &&
                a.strength >= 500 &&
                !a.isSieging &&
            ...
            if (smallerArmy) {
                ...
                if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Assigned ${smallerArmy.strength} troops to move to ${stageId}`);
            }
        ...
        mission.stage = 'MOVING';
        if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Assigned ${candidateArmy.strength} troops to defend ${stageId}`);
    }
    ...
    if (mission.stage === 'MOVING') {
        ...
        if (army.roadId === roadId && army.stageIndex === stageIndex) {
            mission.stage = 'GARRISONING';
            if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Army arrived at ${stageId}`);
            return;
        }
        ...
        if (army.locationType === 'LOCATION') {
            // Enter the road
            const targetRoad = state.roads.find(r =>
                r.id === roadId &&
            ...
            if (targetRoad) {
                ...
                army.isSpent = true;
                if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Army entering road ${roadId} from ${army.locationId}`);
            }
    ...
    if (mission.stage === 'GARRISONING') {
        ...
        if (!shouldFortify || stage.naturalDefense) {
            if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Garrison established at ${stageId} (natural defense: ${stage.naturalDefense || 0})`);
        ...
        if (stage.fortificationLevel && stage.fortificationLevel >= 1) {
            if (DEBUG_AI) console.log(`[AI ROAD_DEFENSE ${faction}] Fortification complete at ${stageId}`);
            mission.status = 'COMPLETED';
            return;
        }
