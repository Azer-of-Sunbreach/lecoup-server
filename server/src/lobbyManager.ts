...
export class LobbyManager {
    ...
    private playerToLobby: Map<string, string> = new Map(); // odId -> gameCode
    ...
    leaveLobby(socketId: string): { lobby?: GameLobby; wasHost: boolean } {
        const code = this.playerToLobby.get(socketId);
        if (!code) return { wasHost: false };

        const lobby = this.lobbies.get(code);
        if (!lobby) return { wasHost: false };
    ...
    selectFaction(socketId: string, faction: FactionId): { success: boolean; lobby?: GameLobby; error?: string } {
        const code = this.playerToLobby.get(socketId);
        if (!code) return { success: false, error: 'Not in a game' };

        const lobby = this.lobbies.get(code);
        if (!lobby) return { success: false, error: 'Game not found' };
    ...
    setPlayerReady(socketId: string, isReady: boolean): { success: boolean; lobby?: GameLobby; error?: string } {
        const code = this.playerToLobby.get(socketId);
        if (!code) return { success: false, error: 'Not in a game' };
    ...
    canStartGame(code: string): { canStart: boolean; reason?: string } {
        const lobby = this.lobbies.get(code);
        if (!lobby) return { canStart: false, reason: 'Game not found' };
    ...
    startGame(code: string): { success: boolean; lobby?: GameLobby; error?: string } {
