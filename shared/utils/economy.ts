...
import { Location, Army, Character, LocationType, RuralCategory, FactionId } from '../types';
import { BONUS_HUNTING_ONLY, BONUS_FISHING_HUNTING } from '../constants';

export const calculateEconomyAndFood = (locs: Location[], armies: Army[], chars: Character[], roads: any[]) => {
    ...
    return tempLocs.map(loc => {
        if (loc.type === LocationType.CITY) {
            ...
            let managerBonus = 0;
            chars.filter(c => c.locationId === loc.id && c.status !== 'DEAD' && c.faction === loc.faction).forEach(leader => { if (leader.stats.ability.includes('MANAGER') && loc.type === LocationType.CITY) managerBonus += 20; });
