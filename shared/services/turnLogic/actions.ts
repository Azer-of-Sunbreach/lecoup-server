...
export const processInsurrections = (
    ...
    nextLocations = nextLocations.map(loc => {
        if (loc.stability < 50 && loc.faction !== FactionId.NEUTRAL && loc.population > 1000) {
            ...
            const isBlocked = isNeutralInsurrectionBlocked(loc, currentTurn);
...
export const processConstruction = (state: GameState): { locations: Location[], roads: any[], armies: Army[], logs: LogEntry[] } => {
    ...
    nextLocations = nextLocations.map(l => {
        ...
        if (l.activeConstruction) {
            ...
            if (builder) referenceFaction = builder.faction;
        }
    ...
    nextRoads = nextRoads.map(r => ({
        ...
        stages: r.stages.map(s => {
            ...
            if (s.activeConstruction) {
                ...
                if (builder) builderFaction = builder.faction;
            }

            const hostileArmiesPresent = nextArmies.some(a =>
                a.locationType === 'ROAD' &&
                a.roadId === r.id &&
...
export const processAutoCapture = (
    ...
    playerFaction: FactionId,
    currentTurn: number = 1
): { locations: Location[], roads: any[], characters: Character[], logs: LogEntry[], tradeNotification: any } => {
    let nextLocations = [...locations];
    let nextRoads = [...roads];
    let nextCharacters = [...characters];
    const logs: LogEntry[] = [];
    ...
    nextLocations = nextLocations.map(loc => {
        ...
        if (invaders.length > 0 && defenders.length === 0) {
            ...
            const governor = nextCharacters.find(c => c.locationId === loc.id && c.status === CharacterStatus.GOVERNING && c.faction === previousFaction);
            ...
            if (governor) {
                ...
                const validationResult = validateGovernorStatus(governor, updatedLoc, nextLocations, nextRoads, currentTurn);
