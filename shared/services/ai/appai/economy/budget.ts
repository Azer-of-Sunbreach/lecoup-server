 ...
 * Shifts budget toward recruitment while reserving gold for fortifications and diplomacy.
 */
export function applyRepublicanEarlyGameOverride(
    ...
    if (faction === FactionId.REPUBLICANS && turn <= 4) {
        ...
        // Early turns: prioritize recruitment but reserve for fortifications and insurrections
        if (turn <= 2) {
            // Turn 1-2: Need 100 gold for fortifications (2 x 25 for Pikes, 2 x 50 for Stone Tower)
            budget.allocations.recruitment = Math.floor(totalAvail * 0.6);
            budget.allocations.diplomacy = Math.floor(totalAvail * 0.25);
            budget.allocations.fortification = Math.floor(totalAvail * 0.15);
        } else {
            // Turn 3-4: Focus more on recruitment
