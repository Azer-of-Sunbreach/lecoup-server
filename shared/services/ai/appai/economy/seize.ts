...
import { GameState, FactionId, Location, LocationType, FACTION_NAMES, LogEntry } from '../../../../types';
 ...
 */
export function handleSeizeActions(
    faction: FactionId,
    state: GameState,
    locations: Location[]
): SeizeResult {
    ...
    for (const city of myLocations.filter(l => l.type === LocationType.CITY)) {
        // Famine imminent: low stock AND negative income
        const isFamineImminent = city.foodStock < 50 && city.foodIncome < 0;

        if (!isFamineImminent) continue;

        // Find linked rural area
        const rural = locations.find(l => l.id === city.linkedLocationId);
        if (!rural || rural.faction !== faction) continue;

        // Check seize eligibility
        if (rural.stability < 15) continue;
        if (rural.actionsTaken?.seizeFood) continue; // Already seized this turn
        ...
        if (lIdx !== -1) {
            locations[lIdx] = {
                ...locations[lIdx],
                stability: Math.max(0, locations[lIdx].stability - REQUISITION_STABILITY_PENALTY),
                actionsTaken: {
            ...
            if (cIdx !== -1) {
                locations[cIdx] = {
                    ...locations[cIdx],
                    foodStock: (locations[cIdx].foodStock || 0) + REQUISITION_AMOUNT
                };
            }
    ...
    const hasCriticalNeed = state.aiState?.[faction]?.missions?.some(m =>
        (m.type === 'CAMPAIGN' && m.status === 'ACTIVE') ||
        (m.type === 'INSURRECTION' && m.status === 'PLANNING')
    ) || currentGold < 50;
    ...
    if (currentGold < 100 && hasCriticalNeed) {
        ...
        for (const city of eligibleCities) {
            if (city.stability < 15) continue; // Safety check
