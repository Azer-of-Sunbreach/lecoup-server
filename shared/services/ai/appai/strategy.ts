...
export const updateMissions = (
    ...
    // D. Insurrection defense missions (NEW: detect threats from logs + stability)
    const insurrectionThreats = detectInsurrectionThreats(state, faction);
    if (insurrectionThreats.length > 0) {
        generateGarrisonMissions(state, faction, insurrectionThreats, activeMissions);
    }
    // Legacy counter-insurrection (kept for ON_MISSION spies fallback)
    generateCounterInsurrectionMissions(state, faction, activeMissions);

    // E. Diplomacy missions (insurrection, negotiate, stabilize)
