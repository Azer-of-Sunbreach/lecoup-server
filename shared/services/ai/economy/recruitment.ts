...
import { FactionId, Location, Army, Road, RoadQuality, Character } from '../../../types';
import { RECRUIT_COST, RECRUIT_AMOUNT } from '../../../constants';
import { getConscriptionPrioritizedLocations } from '../leaders/recruitment/ConscriptionPrioritizer';
import { CONSCRIPTION_GOLD_COST, CONSCRIPTION_STABILITY_COST } from '../../domain/military/conscription';
import { 
    InsurrectionAlert, 
    RecruitmentResult, 
    RecruitmentBudgetInfo, 
    RecruitmentPersonalityInfo 
} from './types';
...
interface RecruitmentTarget {
    location: Location;
    priority: number;
    reason: 'INSURRECTION_ADJACENT' | 'INSURRECTION_DIRECT' | 'MASSING' | 'THREAT' | 'INCOME';
 ...
 */
function findRecruitmentSourcesForThreat(
    threatenedLocationId: string,
    locations: Location[],
    faction: FactionId
): Location[] {
    const threatenedLoc = locations.find(l => l.id === threatenedLocationId);
    if (!threatenedLoc) return [];
 ...
 */
function calculateRecruitmentPriority(
...
export function handleRecruitment(
    ...
    armies: Army[],
    roads: Road[],
    budget: RecruitmentBudgetInfo,
    profile: RecruitmentPersonalityInfo,
    turn: number,
    currentGold: number,
    ...
    const conscriptionResult = getConscriptionPrioritizedLocations(faction, { locations, characters: updatedCharacters } as any);
    ...
    const ownedLocations = locations.filter(l =>
        l.faction === faction &&
        l.population >= 2000 &&
    ...
    for (const loc of ownedLocations) {
        ...
        let reason: RecruitmentTarget['reason'] = 'INCOME';
        if (garrison > 500) {
            reason = 'MASSING';
    ...
    const uniqueTargets = targets.filter(t => {
        if (seenLocations.has(t.location.id)) return false;
    ...
    for (const target of uniqueTargets) {
        ...
        const actionCost = isConscription ? CONSCRIPTION_GOLD_COST : RECRUIT_COST;
        ...
        if (isConscription) {
            ...
            loc.stability = Math.max(0, loc.stability - CONSCRIPTION_STABILITY_COST);
            ...
            console.log(`[AI RECRUITMENT ${faction}] Conscripted in ${loc.name} (-${CONSCRIPTION_GOLD_COST}g, -${CONSCRIPTION_STABILITY_COST} stab)`);
            conscriptionsPerformed++;
        } else {
            console.log(`[AI RECRUITMENT ${faction}] Recruited in ${loc.name} (-${RECRUIT_COST}g)`);
        ...
        if (target.reason === 'INSURRECTION_ADJACENT' && target.linkedThreatenedLocationId) {
            ...
            const road = roads.find(r =>
                (r.from === loc.id && r.to === target.linkedThreatenedLocationId) ||
                (r.to === loc.id && r.from === target.linkedThreatenedLocationId)
            );

            if (road && road.quality === RoadQuality.LOCAL) {
                // Find the army we just recruited into (or created)
                const armyToMove = armies.find(a =>
                    a.faction === faction &&
                    a.locationId === loc.id &&
                    a.locationType === 'LOCATION' &&
