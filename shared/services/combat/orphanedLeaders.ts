...
import { Character, CharacterStatus, CombatState, Army, Location, Road, FactionId } from '../../types';
 ...
 */
export const processOrphanedLeaders = (
    removedArmyIds: string[],
    survivingArmies: Army[],
    characters: Character[],
    context: OrphanedLeaderContext
): OrphanedLeaderResult => {
    ...
    const orphanedWinningLeaders = updatedCharacters.filter(c =>
        c.armyId &&
    ...
    orphanedWinningLeaders.forEach(leader => {
        if (combat.locationId) {
            ...
            updatedCharacters = updatedCharacters.map(c =>
                c.id === leader.id
                    ? { ...c, status: CharacterStatus.AVAILABLE, armyId: null, locationId: combat.locationId! }
                    : c
            );
            ...
            const survivingArmyAtStage = survivingArmies.find(a =>
            ...
            if (survivingArmyAtStage) {
                ...
                if (nearestRural) {
                    updatedCharacters = updatedCharacters.map(c =>
                        c.id === leader.id
                            ? { ...c, status: CharacterStatus.AVAILABLE, armyId: null, locationId: nearestRural.id }
                            : c
                    );
                }
            }
 ...
 */
function findNearestRural(roadId: string, locations: Location[], roads: Road[]): Location | null {
    ...
    if (road) {
        ...
        if (fromLoc && fromLoc.linkedLocationId) {
            ...
            if (linkedRural) return linkedRural;
        }
