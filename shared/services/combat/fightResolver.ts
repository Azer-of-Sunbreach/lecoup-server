...
import { GameState, Army, Character, Location, Road, CombatState, FactionId, GameStats, CharacterStatus, GovernorPolicy } from '../../types';
import { FORTIFICATION_LEVELS } from '../../constants';
import { applySequentialLosses, calculateCombatStrength } from './powerCalculation';
import { processLeaderSurvival } from './leaderSurvival';
import { processOrphanedLeaders } from './orphanedLeaders';
import { getArmiesAtCombatLocation, getLocationName } from './helpers';
...
export const resolveFight = (
    ...
    const actualAttackers = getArmiesAtCombatLocation(combat.attackerFaction, newArmies, combat);
    const actualDefenders = getArmiesAtCombatLocation(combat.defenderFaction, newArmies, combat);

    const attStr = calculateCombatStrength(actualAttackers, newCharacters, 0);
    ...
    const defStr = calculateCombatStrength(actualDefenders, newCharacters, effectiveDefense);

    const attWin = attStr > defStr;
    const locationName = getLocationName(combat, locations, roads);
    ...
    if (attWin) {
        ...
        const survivalResult = processLeaderSurvival(
            actualDefenders.map(a => a.id),
            false,
            newCharacters,
            { combat, attackerWon: true, locations }
        );
        newCharacters = survivalResult.updatedCharacters;
        survivalResult.logMessages.forEach(msg => logMsg += ` ${msg}`);
        if (survivalResult.logEntries) logEntries.push(...survivalResult.logEntries);
        ...
        const { updatedArmies: survivingAttackers } = applySequentialLosses(actualAttackers, losses);
            ...
            newLocations = newLocations.map(l => {
                if (l.id === combat.locationId) {
                    ...
                    if (governor) {
                        ...
                        const validation = validateGovernorStatus(governor, updatedLoc, newLocations, newRoads, 0); // Turn 0?
        ...
        const survivalResult = processLeaderSurvival(
            actualAttackers.map(a => a.id),
            true,
            newCharacters,
            { combat, attackerWon: false, locations }
        );
        newCharacters = survivalResult.updatedCharacters;
        survivalResult.logMessages.forEach(msg => logMsg += ` ${msg}`);
        ...
        const { updatedArmies: survivingDefenders } = applySequentialLosses(actualDefenders, losses);
    ...
    const orphanedResult = processOrphanedLeaders(
        removedArmyIds,
        newArmies,
        newCharacters,
        { combat, winningFaction, locations, roads }
    );
