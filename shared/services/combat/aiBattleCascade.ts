            ...
            if (battle.locationId) {
                newLocations = newLocations.map(l => {
                    if (l.id === battle.locationId) {
                        ...
                        const governor = newCharacters.find(c => c.locationId === battle.locationId && c.status === CharacterStatus.GOVERNING && c.faction === l.faction);

                        if (governor) {
                            const validation = validateGovernorStatus(governor, updatedLoc, newLocations, newRoads, 0);

                            if (!validation.isValid) {
                                newCharacters = newCharacters.map(c => c.id === validation.character.id ? validation.character : c);
                                if (validation.log) logMessages.push(validation.log.message);
                            }
                        }
