...
import { GameState, Army, Road } from '../../types';
 ...
 */
export function executeRetreat(state: GameState, armyId: string): RetreatResult {
 ...
 */
function handleRoadRetreat(army: Army): Army {
    const newDirection = (army.direction === 'FORWARD' ? 'BACKWARD' : 'FORWARD') as 'FORWARD' | 'BACKWARD';
 ...
 */
function handleLocationRetreat(army: Army, state: GameState): Army {
    let updatedArmy = { ...army };
    ...
    let newTargetId: string | null | undefined = currentOrigin;
...
function findConnectingRoad(army: Army, targetId: string, roads: Road[]): Road | undefined {
    return roads.find(r =>
        (r.from === army.locationId && r.to === targetId) ||
        (r.to === army.locationId && r.from === targetId) ||
        (army.locationType === 'ROAD' && r.id === army.roadId)
    );
}

function handleRoadTargetChange(army: Army, road: Road, newTargetId: string): Army {
...
function moveOntoRoad(army: Army, road: Road, newTargetId: string): Army {
