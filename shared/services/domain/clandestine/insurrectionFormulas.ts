 ...
 */
export const AI_ESTIMATION_DEFAULTS = {
 ...
 */
export function getSpontaneousInsurrectionProbability(stability: number): number {
    if (stability >= 50) return 0;
    if (stability >= 40) return 0.25;
    if (stability >= 30) return 0.33;
    if (stability >= 20) return 0.50;
    if (stability >= 10) return 0.75;
    return 1.0; // 0-9%
}
 ...
 */
export function estimateSpontaneousInsurgents(
    stability: number,
    population: number,
    isCity: boolean
): number {
 ...
 */
export function estimateNeutralInsurgents(
    population: number,
    clandestineOps: number,
    resentmentVsController: number,
    stability: number,
    isCity: boolean
): number {
 ...
 */
export function estimateNeutralInsurgentsForAI(location: Location): number {
    const isCity = location.type === 'CITY';
    const resentment = (location.resentment as Record<string, number> | undefined)?.[location.faction] || 0;

    return estimateNeutralInsurgents(
        location.population,
        AI_ESTIMATION_DEFAULTS.CLANDESTINE_OPS,
        resentment || AI_ESTIMATION_DEFAULTS.RESENTMENT_VS_CONTROLLER,
        location.stability,
        isCity
    );
}
 ...
 */
export function calculateProjectedStability(
 ...
 */
export function estimateGrandInsurgents(
    goldInvested: number,
    population: number,
    stability: number,
    clandestineOps: number,
    resentmentVsController: number,
    resentmentVsInstigator: number,
    hasFirebrand: boolean = false
): number {
 ...
 */
export function estimateGrandInsurgentsForAI(
    location: Location,
    instigatorFaction: FactionId
): number {
    const resentmentObj = location.resentment as Record<string, number> | undefined;
    const resentmentVsController = resentmentObj?.[location.faction] || AI_ESTIMATION_DEFAULTS.RESENTMENT_VS_CONTROLLER;
    const resentmentVsInstigator = resentmentObj?.[instigatorFaction] || AI_ESTIMATION_DEFAULTS.RESENTMENT_VS_INSTIGATOR;

    return estimateGrandInsurgents(
        AI_ESTIMATION_DEFAULTS.GOLD_INVESTED,
        location.population,
        location.stability,
        AI_ESTIMATION_DEFAULTS.CLANDESTINE_OPS,
        resentmentVsController,
        resentmentVsInstigator,
        false // Assume no firebrand for safety
    );
}
...
export function calculateRequiredGarrison(
    ...
    marginMultiplier: number = AI_ESTIMATION_DEFAULTS.GARRISON_MARGIN
): number {
 ...
 */
export function calculateGarrisonDeficit(
