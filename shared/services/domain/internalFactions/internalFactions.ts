...
import { Character, CharacterStatus, FactionId, Location, LeaderAbility } from '../../../types';
...
export const KNIGHTLY_COUP_GOLD_COST = 250;
 ...
 */
export function executeKnightlyCoupChoice(
    ...
    const updatedCharacters = characters.map(char => {
        ...
        if (char.id === KNIGHTLY_COUP_ABILITY_DISABLED_LEADER) {
            ...
            return {
                ...char,
                disabledAbilities: [...existingDisabled, 'AGITATIONAL_NETWORKS' as LeaderAbility],
...
export const RABBLE_MISSION_BUDGET = 400;
 ...
 */
export function executeRabbleVictoryChoice(
    characters: Character[],
    locations: Location[],
    playerFaction: FactionId
): RabbleVictoryResult {
    ...
    const updatedCharacters = characters.map(char => {
        ...
        if (RABBLE_ABILITY_GRANTED_LEADERS.includes(char.id)) {
            ...
            return {
                ...char,
                grantedAbilities: [...existingGranted, 'AGITATIONAL_NETWORKS' as LeaderAbility],
 ...
 */
export function hasActiveAbility(character: Character, ability: LeaderAbility): boolean {
 ...
 */
export function getActiveAbilities(character: Character): LeaderAbility[] {
    ...
    return allAbilities.filter(a => !disabledAbilities.includes(a)) as LeaderAbility[];
}
...
export const MERCHANT_DOMINATION_GOLD_COST = 250;
 ...
 */
export function executeMerchantDominationChoice(
    characters: Character[],
    locations: Location[],
    playerFaction: FactionId
): MerchantDominationResult {
    // Validation: Player must be Republicans
    if (playerFaction !== FactionId.REPUBLICANS) {
        return {
            success: false,
    ...
    const updatedCharacters = characters.map(char => {
        ...
        if (char.id === MERCHANT_TRAIT_GRANTED_LEADER) {
            ...
            // Avoid duplicates
            if (currentTraits.includes('FREE_TRADER' as any)) {
                return char;
            }
            return {
                ...char,
                stats: {
                    ...char.stats,
                    traits: [...currentTraits, 'FREE_TRADER' as any]
                }
            };
        }
    ...
    locationsToCheck.forEach(locId => {
        ...
        if (locIndex !== -1) {
            ...
            const result = enforceFreeTraderLimits(loc, updatedCharacters);
            if (result.modified) {
