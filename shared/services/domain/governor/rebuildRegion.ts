 ...
 */
export function isRebuildRegionActive(location: Location): boolean {
    return location.governorPolicies?.[GovernorPolicy.REBUILD_REGION] === true;
}
 ...
 */
export function hasRebuildableDamage(location: Location): boolean {
 ...
 */
export function getDamageAmount(location: Location): number {
    if (location.type === LocationType.RURAL) {
 ...
 */
export function shouldDisableRebuildRegion(
    location: Location,
    factionRevenue: number
): { shouldDisable: boolean; reason?: 'revenue_zero' | 'no_damage' } {
 ...
 */
export function processRebuildRegion(governor: Character, location: Location): Location {
