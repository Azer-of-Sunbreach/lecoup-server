...
import { Character, CharacterStatus, FactionId, Location, Road, LogEntry, GovernorPolicy, LogType, LogSeverity } from '../../../types';
import { calculateLeaderTravelTime } from '../leaders';
import { hasIronFistTrait, applyIronFistPolicy } from './makeExamples';
 ...
 */
export function executeAppointGovernor(
    ...
    if (leader.status !== CharacterStatus.AVAILABLE &&
        leader.status !== CharacterStatus.UNDERCOVER &&
        leader.status !== CharacterStatus.GOVERNING) {
 ...
 */
export function executeCancelGovernorAppointment(
    characters: Character[],
    leaderId: string
): Character[] {
 ...
 */
export function getGovernorForLocation(
    characters: Character[],
    locationId: string,
    faction: FactionId
): Character | undefined {
 ...
 */
export function getTravelingGovernor(
    characters: Character[],
    locationId: string,
    faction: FactionId
): Character | undefined {
    return characters.find(c =>
        c.governorMission?.destinationId === locationId &&
 ...
 */
export function processGovernorMissionTravel<T extends Character>(
    characters: T[],
    locations: Location[]
): { characters: T[]; locations: Location[]; logs: LogEntry[] } {
    const logs: LogEntry[] = [];
    const updatedLocationsMap = new Map<string, Location>();
    const locationMap = new Map(locations.map(l => [l.id, l]));

    const processedCharacters = characters.map(c => {
        // Only process leaders traveling for governor mission
        if (c.status !== CharacterStatus.MOVING || !c.governorMission) {
            return c;
        }
        ...
        if (destination.faction === c.faction) {
            ...
            if (hasIronFistTrait(c)) {
                updatedLocationsMap.set(destination.id, applyIronFistPolicy(destination));
            }
    ...
    const finalCharacters = processedCharacters.map(c => {
        if (c.status !== CharacterStatus.GOVERNING) return c;

        // Check if there's another governor in the same location
        const otherGovernors = processedCharacters.filter(other =>
            other.id !== c.id &&
            other.locationId === c.locationId &&
            other.status === CharacterStatus.GOVERNING &&
    ...
    const finalLocations = locations.map(loc =>
        updatedLocationsMap.has(loc.id) ? updatedLocationsMap.get(loc.id)! : loc
    );

    return {
        characters: finalCharacters as T[],
...
// ============================================================================
// GOVERNOR STATUS VALIDATION (ANOMALY FIX)
// ============================================================================

export interface GovernorValidationResult {
    character: Character;
    log?: LogEntry;
    isValid: boolean; // True if remains governor, False if changed (fled/died/demoted)
}
 ...
 */
export function validateGovernorStatus(
    governor: Character,
    location: Location,
    ...
    // Check roads
    roads.forEach(r => {
        if (r.from === location.id) adjacentIds.add(r.to);
        if (r.to === location.id) adjacentIds.add(r.from);
    });
